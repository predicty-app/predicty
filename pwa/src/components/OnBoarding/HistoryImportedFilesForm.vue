...
const onlyTodayImportsHistoryList = computed<ImportType[]>(() => importsList.value.filter((item: ImportType) => hGetParseDate(item.startedAt) === hGetParseDate()));
...
<template>
  ...
  <div v-if="isComponentMounted" class="flex flex-col gap-y-6">
    <div class="flex justify-between items-center">
      <HeaderText class="px-5" :header-title="t('components.on-boarding.history-imported-files-form.header.title')" />
      <HeaderText class="px-5" :header-description="t('components.on-boarding.history-imported-files-form.header.description', {
        ...
        <div class="grid grid-flow-row grid-rows-auto gap-2">
          <div class="grid grid-cols-[32px_1fr_1fr] items-center gap-2">
            ...
            <h4 class="text-base font-bold">
              {{ t("components.on-boarding.history-imported-files-form.import-from") }}
              {{
            ...
            <div class="flex direction-column" :class="{
              ...
              <ButtonForm isSmall :class="{ 'w-1/2': importFile.__typename !== 'FileImport' }">
                {{ t("components.on-boarding.history-imported-files-form.button.revert") }}
              </ButtonForm>
              <ButtonForm isSmall class="ml-3" v-if="importFile.__typename === 'FileImport'">{{
                t("components.on-boarding.history-imported-files-form.button.show") }}</ButtonForm>
            </div>
          </div>
              ...
              <p>
                <span class="text-imports-green font-bold">{{ importFile.result.createdAds }}
                  {{
                    ...
                    ? t("components.on-boarding.history-imported-files-form.ad")
                    : t("components.on-boarding.history-imported-files-form.ads")
                  }}</span>
                {{
                  importFile.result.createdAds === 1
                  ? t("components.on-boarding.history-imported-files-form.element-added")
                  : t("components.on-boarding.history-imported-files-form.elements-added")
                }}.
              </p>
              <p>
                <span class="text-imports-green font-bold">{{ importFile.result.createdCampaigns }}
                  {{
                    ...
                    ? t("components.on-boarding.history-imported-files-form.campaign")
                    : t("components.on-boarding.history-imported-files-form.campaigns")
                  }}</span>
                {{
                  importFile.result.createdCampaigns === 1
                  ? t("components.on-boarding.history-imported-files-form.element-added")

...
async function handleSubmitForm() {
  ...
  termsError.value =
    termsModel.value.fileVersionId === 0
 ...
 */
function handleChangeStateCheckbox(
  type: "fileVersionId" | "newsletter",
  stateValue: boolean
) {
  if (type === "fileVersionId") {
    termsModel.value.fileVersionId = stateValue
...
<template>
  <div v-if="isComponentMounted" class="flex flex-col gap-y-6">
    ...
    <div class="flex gap-x-2 text-xs">
      <CheckboxForm
        :is-checked="termsModel.fileVersionId > 0"
        color="#272727"
        border-color="#272727"
      ...
      <span
        ...
        @click="
          handleChangeStateCheckbox(
            'fileVersionId',
            !(termsModel.fileVersionId > 0)
          )
        "
      >
        I agree with <span class="font-bold"> Terms and conditionsI</span></span
      >
      <div v-if="termsError" class="text-red-100 text-xs">{{ termsError }}</div>
    </div>
    <div class="flex gap-x-2 text-xs">
      <CheckboxForm
        :is-checked="termsModel.newsletter"
        color="#272727"
        border-color="#272727"
      ...
      <span
        class="cursor-pointer"
        @click="handleChangeStateCheckbox('newsletter', !termsModel.newsletter)"
      >
        I want to receive new features, thus agree to
        <span class="font-bold"
          >Rules of receiving communication by email</span
        ></span
      >
    </div>

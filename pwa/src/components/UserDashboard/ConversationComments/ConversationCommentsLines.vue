...
function parseDateByDay(day: number): string {
  const dateToParse = `${props.conversationDate.slice(6, 10)}-${props.conversationDate.slice(3, 5)}-${props.conversationDate.slice(0, 2)}`
...
<template>
  <div ref="conversationCommentLine" class="conversation-comments-lines" v-if="isElementVisible">
    <ConversationCommentsLine :key="parseDateByDay(day - 1)" :conversation-date="parseDateByDay(day - 1)" v-for="day in 7" />
  </div>
</template>

  ...
  <div class="absolute w-full h-[99%] left-0 top-0">
    <ScrollbarPanel class="h-[99%]" :scroll-x="globalStore.scrollParams.x">
      <div v-if="Object.keys(globalStore.dictionaryFirstDaysWeek).length > 0"
        ...
        <ConversationCommentsLines :key="`lines_${item}`" class="col-start-dynamic col-end-dynamic"
          v-for="(item, index) in globalStore.currentsCountWeeks" :style="{ '--start': index + 1, '--end': index + 2 }"

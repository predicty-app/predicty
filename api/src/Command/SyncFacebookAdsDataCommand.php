...
#[AsCommand(
    name: 'app:sync:facebook-ads',
    description: 'Synchronize data from Facebook Ads',
)]
class SyncFacebookAdsDataCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $connectedAccount = $this->connectedAccountRepository->findByAccountId($account->getId(), FacebookAdsConnectedAccount::class);

        if ($connectedAccount === null) {
            $io->error('No connected Facebook Ads account found for this user and account id');

            return Command::FAILURE;
        }

        $this->dispatch(new SyncFacebookAds($user->getId(), $account->getId(), $connectedAccount->getId()));

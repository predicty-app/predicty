...
class ImportFixture extends Fixture implements DependentFixtureInterface
{
    ...
    public function load(ObjectManager $manager): void
    {
        ...
        $import1 = new FileImport($user->getId(), $account->getId(), 'dummy-import-1.csv', FileImportType::FACEBOOK_CSV);
        $this->addReference(self::IMPORT_1, $import1);

        $import2 = new FileImport($user->getId(), $account->getId(), 'dummy-import-2.csv', FileImportType::GOOGLE_ANALYTICS_REVENUE);
        $import2->start();
        $this->addReference(self::IMPORT_2, $import2);

        $import3 = new FileImport($user->getId(), $account->getId(), 'dummy-import-3.csv', FileImportType::GOOGLE_ADS_CSV);
        $import3->start();
        $import3->fail('Import failed');
        $this->addReference(self::IMPORT_3, $import3);

        $import4 = new ApiImport($user->getId(), $account->getId(), DataProvider::GOOGLE_ADS);
        $import4->start();
        $this->addReference(self::IMPORT_4, $import4);

        $import5 = new FileImport($user->getId(), $account->getId(), 'dummy-import-1.csv', FileImportType::FACEBOOK_CSV);
        $import5->start();
